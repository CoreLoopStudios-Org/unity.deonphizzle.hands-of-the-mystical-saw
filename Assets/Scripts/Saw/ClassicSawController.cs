...
public class ClassicSawController : MonoBehaviour
{
    [Header("--- Tool State ---")]
    ...
    public Vector3 extendAxis = new Vector3(-1, 0, 0);
    ...
    void Start()
    {
        Debug.Log($"[ClassicSawController] Initializing on '{gameObject.name}' with classic control mapping.");

        // Force classic axes and inversions programmatically
        rootRotationAxis = new Vector3(1, 0, 0);  // Local X-axis
        tiltRotationAxis = new Vector3(0, 0, 1);  // Local Z-axis
        extendAxis = new Vector3(-1, 0, 0);       // Local negative X-axis
        spinAxis = new Vector3(0, 0, 1);
        bladeCutNormal = new Vector3(0, 0, 1);
        invertJoystickX = true;
        invertJoystickY = false;
    ...
    void Update()
    {
        ...
        // 2. Joystick/Gyro Aim (Analog / Smooth aim without axis locking)
        {
            float joyX = 0f;
            float joyY = 0f;
