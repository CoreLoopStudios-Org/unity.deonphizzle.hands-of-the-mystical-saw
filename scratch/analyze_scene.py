...
scene_path = r"C:\Users\User\Documents\GitHub\unity.deonphizzle.hands-of-the-mystical-saw\Assets\ALL-SCENE-IS HERE\StoneGenerator Scene.unity"
...
# Print root GameObjects and check if they have components
print("=== ROOT GAMEOBJECTS & MONOBEHAVIOURS ===")
for go_id, go in game_objects.items():
    # Find its transform to check if father is 0
    is_root = False
    for tr_id, tr in transforms.items():
        if tr["game_object"] == go_id and tr["father"] == "0":
            is_root = True
            break
    if is_root or not is_root: # Print all gameobjects with components
        if go["components"]:
